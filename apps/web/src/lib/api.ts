...
export const api = {
  ...
  disableTwoFactor: (code: string, currentPassword: string) => request<{ user: User }>("/api/me/2fa/disable", { method: "POST", body: JSON.stringify({ code, currentPassword }) }),
  ...
  createRegistrationInvite: (payload: { code?: string; maxUses: number; permissionGroupIds?: string[] }) => request<RegistrationInvite>("/api/admin/registration-invites", { method: "POST", body: JSON.stringify(payload) }),
