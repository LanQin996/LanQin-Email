...
export function AdminPage() {
  ...
  const visibleSections = sectionKeys.filter((key) =>
    hasAnyPermission(user, sectionPermissions[key])
  )
  const rawSection = params.get("section") as Section | null
  const section: Section =
    rawSection && visibleSections.includes(rawSection)
      ? rawSection
  ...
  const overview = useQuery({
    ...
    enabled: !!user && canOverview && section === "overview",
  })
  const users = useQuery({
    ...
    enabled:
      !!user &&
      ((section === "users" && canUsersView) || (section === "mailboxes" && canMailboxesView)),
  })
  const permissionGroups = useQuery({
    ...
    enabled:
      !!user &&
      ((section === "permissionGroups" && canPermissionGroupsView) ||
  ...
  const mailboxes = useQuery({
    ...
    enabled:
      !!user &&
      ((section === "mailboxes" && canMailboxesView) ||
        ((section === "messages" || section === "sendAudit") && canMessagesView)),
  })
  const aliases = useQuery({
    queryKey: ["admin", "aliases"],
    queryFn: api.aliases,
    enabled: !!user && canAliasesView && section === "aliases",
  })
  const settings = useQuery({
    queryKey: ["admin", "settings"],
    queryFn: api.systemSettings,
    enabled: !!user && canSettingsView && (section === "overview" || section === "settings"),
  })
  const deliveryQueue = useQuery({
    queryKey: ["admin", "delivery-queue"],
    queryFn: () => api.adminDeliveryQueue({ limit: 100 }),
    enabled: !!user && canSettingsView && section === "deliveryQueue",
  ...
  const assignablePermissionGroups = (permissionGroups.data?.items || []).filter(
    (group) => group.id !== "pg_super_admin" && group.id !== "pg_regular_user"
  )
  return (
    <ScrollArea className="h-[calc(100svh-3rem)] md:h-svh">
      <main className="p-4 sm:p-6">
        ...
        {section === "mailboxes" && (
          <MailboxesSection mailboxes={mailboxItems} users={userItems} domains={domainItems} />
        )}
        ...
        {section === "deliveryQueue" && (
          <AdminDeliveryQueueSection
            items={deliveryQueue.data?.items || []}
            canUpdate={canSettingsUpdate}
          />
        )}
        {section === "settings" && (
          <SystemSettingsSection settings={settings.data} domains={domainItems} />
        )}
      </main>
    </ScrollArea>
  )
...
function setupChecklist(
  ...
  return [
    ...
    },
    {
      key: "dns",
      title: "完成 DNS 检测",
      detail: dnsReady ? "至少一个域名 DNS 正常" : "配置 MX、SPF、DKIM、DMARC 后执行检测",
      done: dnsReady,
      section: "domains" as Section,
    },
    {
...
function PermissionGroupsSection({
  ...
  const filtered = groups.filter((group) => {
    ...
    return [group.name, group.description, ...group.permissions].some((value) =>
      value.toLowerCase().includes(keyword)
    )
  })
...
function PermissionGroupDialog({
  ...
  const [limits, setLimits] = React.useState<PermissionLimits>(
    group?.limits || defaultPermissionLimits
  )
  React.useEffect(() => {
    if (dialogOpen) {
  ...
  const mutation = useMutation({
    mutationFn: (form: FormData) => {
      ...
      return group
        ? api.updatePermissionGroup(group.id, payload)
        : api.createPermissionGroup(payload)
    },
    onSuccess: () => {
      invalidateAdmin(qc)
      setDialogOpen(false)
...
function PermissionPicker({
  ...
  function toggle(permission: PermissionKey, checked: boolean) {
    onChange(
      checked
        ? Array.from(new Set([...value, permission]))
...
function PermissionLimitEditor({
  value,
  onChange,
}: {
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(event) => update("maxAttachmentMb", event.target.value)}
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            type="number"
            min={0}
            value={value.smtpDailyLimit}
            onChange={(event) => update("smtpDailyLimit", event.target.value)}
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            type="number"
            min={0}
            value={value.smtpMinuteLimit}
            onChange={(event) => update("smtpMinuteLimit", event.target.value)}
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            type="number"
            min={0}
            value={value.imapMinuteLimit}
            onChange={(event) => update("imapMinuteLimit", event.target.value)}
          />
        </div>
        <div className="space-y-2">
...
function DomainDNSDialog({ domain }: { domain: Domain }) {
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[86vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>{domain.name} DNS</DialogTitle>
        </DialogHeader>
...
function AliasesSection({ aliases, domains }: { aliases: Alias[]; domains: Domain[] }) {
  ...
  const update = useMutation({
    mutationFn: ({
      ...
      id: string
      payload: { source: string; destination: string; enabled: boolean }
    }) => api.updateAlias(id, payload),
    onSuccess: () => {
      invalidateAdmin(qc)
      toast({ title: "别名已更新" })
    },
...
function AdminMessagesSection({ mailboxes }: { mailboxes: MailboxType[] }) {
  ...
  const detail = useQuery({
    ...
    queryFn: () => api.adminMessage(selectedId!),
    enabled: !!selectedId,
  })
              ...
              {items.map((message) => (
                <TableRow key={message.id}>
                  ...
                  <TableCell>
                    ...
                    {message.ownerEmail && (
                      <div className="text-xs text-muted-foreground">{message.ownerEmail}</div>
                    )}
                  </TableCell>
                  <TableCell className="max-w-[220px] truncate" title={adminSenderTitle(message)}>
                    {adminSenderDisplayName(message)}
                  </TableCell>
              ...
              {items.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell
                    ...
                    title={item.messageId || item.sentMessageId || ""}
                  >
                    {item.messageId || item.sentMessageId || "-"}
                  </TableCell>
                  <TableCell
...
function AdminDeliveryQueueSection({
  items,
  canUpdate,
}: {
  ...
  const filtered = items.filter(
    (item) =>
      (type === "all" || item.queueType === type) && (status === "all" || item.status === status)
  )
  const retry = useMutation({
    mutationFn: (item: import("@/lib/api-types").AdminDeliveryQueueItem) =>
      api.retryAdminDeliveryQueue(item.queueType, item.id),
    onSuccess: () => {
  ...
  const cancel = useMutation({
    mutationFn: (item: import("@/lib/api-types").AdminDeliveryQueueItem) =>
      api.cancelAdminDeliveryQueue(item.queueType, item.id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin", "delivery-queue"] })
      toast({ title: "任务已取消" })
    },
...
function SystemSettingsSection({
  ...
  const templates = useQuery({
    ...
    queryFn: api.mailTemplates,
    enabled: canViewTemplates,
  })
  const [settingsTab, setSettingsTab] = React.useState<
    "base" | "smtp" | "storage" | "mail" | "externalImap" | "templates" | "security" | "about"
  >("base")
  const maildirHealth = useQuery({
    queryKey: ["admin", "maildir-sync", "health"],
    queryFn: api.maildirSyncHealth,
    enabled: canSettingsView && settingsTab === "storage",
  ...
  const save = useMutation({
    mutationFn: (form: FormData) =>
      api.updateSystemSettings({
        ...
        maildirScanSeconds: fieldNumber(
          form,
          "maildirScanSeconds",
          settings?.maildirScanSeconds || 5
        ),
        ...
        catchAllEnabled,
        mailAutoRefresh,
        mailRefreshSeconds: fieldNumber(
          form,
          "mailRefreshSeconds",
          settings?.mailRefreshSeconds || 30
        ),
        userMailboxApplyEnabled,
        userMailboxDomainIds,
        reservedMailboxPrefixes: fieldValue(
          form,
          "reservedMailboxPrefixes",
          settings?.reservedMailboxPrefixes || ""
        ),
        externalImapEnabled,
        externalImapSecretKey: fieldValue(form, "externalImapSecretKey", ""),
        externalImapSyncSeconds: fieldNumber(
          form,
          "externalImapSyncSeconds",
          settings?.externalImapSyncSeconds || 300
        ),
        externalImapAllowPrivateHosts,
        externalImapGmailClientId: fieldValue(
          form,
          "externalImapGmailClientId",
          settings?.externalImapGmailClientId || ""
        ),
        externalImapGmailClientSecret: fieldValue(form, "externalImapGmailClientSecret", ""),
        externalImapOutlookClientId: fieldValue(
          form,
  ...
  const formKey = settings
    ...
    : "loading"
  const tabs = React.useMemo<{ key: typeof settingsTab; label: string }[]>(
    () => [
      ...(canSettingsView
        ? [
    ...
    [canSettingsView, canViewTemplates]
  )
  React.useEffect(() => {
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        event.preventDefault()
        if (canUpdateSettings) save.mutate(new FormData(event.currentTarget))
      }}
      className="space-y-6"
    >
      <div className="flex flex-wrap gap-2 rounded-lg border bg-card p-2">
        {tabs.map((tab) => (
          <Button
            key={tab.key}
            type="button"
            variant={settingsTab === tab.key ? "default" : "ghost"}
            size="sm"
      ...
      {settingsTab === "storage" && (
        <div className="space-y-6">
          ...
          <MaildirSyncHealthCard
            health={maildirHealth.data}
            loading={maildirHealth.isLoading}
            error={maildirHealth.error}
            onRefresh={() => maildirHealth.refetch()}
            refreshing={maildirHealth.isFetching}
            fallbackRoot={settings?.maildirRoot || ""}
          />
        </div>
      )}
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={(value) =>
                              setUserMailboxDomainIds((items) =>
                                value === true
                                  ? Array.from(new Set([...items, domain.id]))
                                  : items.filter((id) => id !== domain.id)
                              )
                            }
                          />
      ...
      {settingsTab === "templates" && canViewTemplates && (
        <MailTemplatesPanel
          templates={templates.data?.items || []}
          loading={templates.isLoading}
          canUpdate={canUpdateTemplates}
          canReset={canResetTemplates}
        />
      )}
...
function RegistrationInvitesPanel({ canUpdate }: { canUpdate: boolean }) {
  ...
  const invites = useQuery({
    ...
    queryFn: api.registrationInvites,
  })
  ...
  return (
    <div className="space-y-3 border-t pt-5">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => removeItem && remove.mutate(removeItem.id)}
      />
    </div>
  )
}

function MaildirSyncHealthCard({
  health,
  loading,
  error,
  onRefresh,
      ...
      <CardHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant={
                health?.running ? "default" : health?.workerStarted ? "outline" : "secondary"
              }
            >
              {health?.running
                ? "运行中"
...
function maildirCounterRows(counters?: Record<string, number | undefined>) {
  return [
    {
      key: "directoriesChecked",
      label: "检查目录",
      value: counterValue(counters, "directoriesChecked"),
    },
    {
      key: "directoriesScanned",
      label: "扫描目录",
      value: counterValue(counters, "directoriesScanned"),
    },
...
function formatRunDuration(
  currentRun?: MaildirSyncHealth["currentRun"],
  lastRun?: MaildirSyncHealth["lastRun"]
) {
  if (currentRun) {
      ...
      <CardContent className="space-y-4 text-sm">
          ...
          {projectTag ? (
            <div className="flex flex-wrap items-center gap-2">
              <Button
                type="button"
                variant="outline"
                className="h-11 justify-start px-4 text-base font-normal"
                asChild
              >
        ...
        <AboutRow label="帮助">
          <div className="flex flex-wrap gap-3">
            ...
            >
              <a
                href={`${projectRepositoryUrl}/blob/main/LICENSE`}
                target="_blank"
                rel="noreferrer"
              >
  ...
  return (
    <Card>
      <CardContent className="flex items-center gap-3 p-4 sm:gap-4 sm:p-5">
        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-muted text-foreground sm:h-10 sm:w-10">
          {icon}
        </div>
        <div className="min-w-0">
...
function assignableUserGroupIDs(user: AdminUser) {
  return (user.permissionGroupIds || []).filter(
    (id) => id !== "pg_super_admin" && id !== "pg_regular_user"
  )
}
...
function MailboxActions({
  ...
  return (
    <>
      ...
      {canUpdate && (
        <EditMailboxDialog mailbox={mailbox} users={users} open={open} onOpenChange={setOpen} />
      )}
    </>
  )
}
...
function ResetPasswordDialog({
  ...
  const mut = useMutation({
    mutationFn: (form: FormData) =>
      api.resetUserPassword(user.id, String(form.get("password") || "")),
    onSuccess: () => {
      onOpenChange(false)
      toast({ title: "密码已重置" })
    },
...
function EditMailboxDialog({
  ...
  const [status, setStatus] = React.useState(mailbox.status)
  React.useEffect(() => {
    setUserId(mailbox.userId)
...
function DNSPanel({ domain, embedded = false }: { domain?: Domain; embedded?: boolean }) {
  ...
  const records = useQuery({
    queryKey: ["dns-records", domain?.id],
    queryFn: () => api.dnsRecords(domain!.id),
    enabled: !!domain,
  })
  const check = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast({
        title: res.status === "ok" ? "DNS 检测通过" : "DNS 检测未通过",
        description: Object.values(res.checks)
          .map((c) => c.message)
          .join("；"),
      })
    },
...
function dnsDescription(record: DNSRecord): string {
  if (record.type === "TXT" && record.name.startsWith("_dmarc"))
    return "声明域名的 DMARC 策略（如何处理未通过 SPF/DKIM 验证的邮件）。"
  if (record.type === "TXT" && record.value.includes("DKIM1"))
    return "DKIM 公钥。收件服务器用此密钥验证邮件是否由你发出。"
  if (record.type === "TXT" && record.value === "v=spf1 a -all")
    return "授权邮件服务器主机名用于 SMTP HELO，避免对方报告 HELO 未发布 SPF。"
